...
import { Sparkles, Calendar, Search, Loader2, RefreshCw, Sun, Moon } from "lucide-react";
...
export default function TopBar({
  ...
  return (
    <header
      id="topbar"
      className="bg-slate-950/40 backdrop-blur-md border-b border-white/5 px-8 py-4 flex items-center justify-between sticky top-0 z-20 space-x-4"
    >
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className="relative">
            ...
            <input
              ...
              disabled={isParsing}
              className="w-full bg-slate-900/40 border border-white/5 focus:border-indigo-500/80 rounded-xl py-3 pl-11 pr-32 text-sm text-zinc-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 transition-all duration-200"
            />
            <div className="absolute right-2 top-2">
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-zinc-800 disabled:text-zinc-500 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-all flex items-center space-x-1.5 shadow-md shadow-indigo-600/10 cursor-pointer"
              >
                {isParsing ? (
                  <>
